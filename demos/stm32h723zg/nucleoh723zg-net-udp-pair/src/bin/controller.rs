...
async fn run_socket(
    ...
    let mut rxtx = RxTxWorker::new(&STACK, socket, EdgeFrameProcessor::new_controller(1), (), consumer, endpoint);

    loop {
        _ = rxtx.run(InterfaceState::Active { net_id: 1, node_id: CENTRAL_NODE_ID }, scratch_buf).await;
    }
}
